...
sourceString=${1}
targetString=${2}
grep -rl "${sourceString}" --exclude-dir='.*/Carthage/Pods/fastlane/build' --exclude='.DS_Store' --exclude-dir='*Firebase*' --exclude='*rename*' ./ | xargs -I{} sed -i \'\' "'s/$sourceString/$targetString/g'" {} 
}
...
function replaceContentsOfFiles {
  ...
  SAVEIFS=$IFS
  IFS=$(echo -en "\n\b")
  for it in $(grep -RIl --exclude-dir={*Pods*,*Carthage*,.*,*build*,*fastlane*,*Firebase*} --exclude={*rename*,*DS_Store*,*.xcuserstate,*.png,*.jpg,*.json,*.appiconset,*.a,*.svg} ${OLD_PROJECT_NAME} *)
  do
    if [ -h "$it" ]; then
      continue
    fi
